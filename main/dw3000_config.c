...
size_t dw3000_config_format_json(char *out, size_t out_cap)
{
    ...
    int n = snprintf(out, out_cap, "{\"addr\":%u,\"pan\":%u,\"ant\":%u,\"twr\":%lu}", (unsigned)cfg->addr,
...
esp_err_t dw3000_config_init(void)
{
    ...
    if (!valid_config(&s_cfg)) {
        ...
        ESP_LOGI(TAG, "loaded addr=0x%04X pan=0x%04X ant=%u twr=%lu us", (unsigned)s_cfg.addr,
...
esp_err_t dw3000_config_set(const dw3000_config_t *cfg)
{
    ...
    s_cfg = *cfg;
    ESP_LOGI(TAG, "saved addr=0x%04X pan=0x%04X ant=%u twr=%lu us", (unsigned)s_cfg.addr, (unsigned)s_cfg.panid,
             (unsigned)s_cfg.antenna_delay, (unsigned long)s_cfg.twr_delay_us);
    return ESP_OK;
}
